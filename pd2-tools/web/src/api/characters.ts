...
export const charactersAPI = {
  ...
  /**
   * Analyze character build and get optimization recommendations
   */
  async analyzeCharacter(
    name: string,
    gameMode: string = "softcore",
    season?: number,
    includeMetaComparison: boolean = true
  ): Promise<BuildOptimization> {
    return apiClient.get<BuildOptimization>(
      `${API_ENDPOINTS.character}/${name}/analyze`,
      {
        gameMode,
        season,
        includeMetaComparison: includeMetaComparison.toString(),
      }
    );
  },
};
