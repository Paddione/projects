...
export interface ErrorDetails {
  ...
  category: 'validation' | 'authentication' | 'authorization' | 'database' | 'network' | 'system' | 'business' | 'unknown';
  recoverable: boolean;
  retryable: boolean;
}
...
export class CentralizedErrorHandler {
   ...
   */
  async handleError(error: Error | ErrorDetails, context?: Partial<ErrorContext>): Promise<void> {
   ...
   */
  private normalizeError(error: Error | ErrorDetails, context?: Partial<ErrorContext>): ErrorDetails {
  ...
  private categorizeError(error: Error): string {
    ...
    if (name.includes('auth') || message.includes('auth') || message.includes('token')) return 'AUTH_ERROR';
    if (message.includes('database') || message.includes('connection') || message.includes('query')) return 'DATABASE_ERROR';
    if (message.includes('network') || message.includes('timeout') || message.includes('fetch')) return 'NETWORK_ERROR';
    if (message.includes('permission') || message.includes('forbidden') || message.includes('access')) return 'PERMISSION_ERROR';
  ...
  private determineSeverity(error: Error): 'low' | 'medium' | 'high' | 'critical' {
    ...
    if (message.includes('database') || message.includes('auth') || message.includes('security')) return 'high';
  ...
  private determineCategory(error: Error): ErrorDetails['category'] {
    ...
    if (message.includes('system') || message.includes('memory')) return 'system';
  ...
  private isRecoverable(error: Error): boolean {
    ...
    if (message.includes('syntax') || message.includes('reference') || message.includes('type')) return false;
    if (message.includes('permission') || message.includes('forbidden')) return false;
    
    // Recoverable errors
    if (message.includes('network') || message.includes('timeout')) return true;
    if (message.includes('database') || message.includes('connection')) return true;
   ...
   */
  async getErrorStatistics(timeRange: 'hour' | 'day' | 'week' = 'hour'): Promise<{
    totalErrors: number;
