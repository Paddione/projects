...
export class CoverageReporter {
  ...
  private generateHtmlReport(
    ...
    const outputPath = path.join(outDir, 'coverage-report.html');
...
<body>
  ...
  private generateLcovReport(coverage: CoverageReport, options: CoverageReportOptions): string {
    const outDir = options.outputDir ?? this.outputDir;
    this.ensureDirectoryExists(outDir);
    const outputPath = path.join(outDir, 'lcov.info');
  ...
  private generateJsonReport(
    ...
    const outDir = options.outputDir ?? this.outputDir;
    this.ensureDirectoryExists(outDir);
    const outputPath = path.join(outDir, 'coverage-report.json');
  ...
  private generateXmlReport(coverage: CoverageReport, options: CoverageReportOptions): string {
    const outDir = options.outputDir ?? this.outputDir;
    this.ensureDirectoryExists(outDir);
    const outputPath = path.join(outDir, 'coverage-report.xml');
...
<coverage line-rate="${(coverage.overall.lines.percentage / 100).toFixed(4)}" 
  ...
  private generateTextReport(
    ...
    const outDir = options.outputDir ?? this.outputDir;
    this.ensureDirectoryExists(outDir);
    const outputPath = path.join(outDir, 'coverage-report.txt');
...
`;
  ...
  private generateBadgeReport(coverage: CoverageReport, options: CoverageReportOptions): string {
    const outDir = options.outputDir ?? this.outputDir;
    this.ensureDirectoryExists(outDir);
    const outputPath = path.join(outDir, 'coverage-badge.svg');
  ...
  private calculateTrends(
    ...
    // Compare current coverage against the most recent historical point
    const previous = historicalData[historicalData.length - 1]!.coverage;
