...
scripts=("scripts/setup.sh" "scripts/deploy.sh" "scripts/test-runner.sh")
...
for script in "${shell_scripts[@]}"; do
    if [ -x "scripts/${script}.sh" ]; then
        print_success "Shell script 'scripts/${script}.sh' exists and is executable"
    else
        print_error "Shell script 'scripts/${script}.sh' is missing or not executable"
        errors=$((errors + 1))
    fi
done
