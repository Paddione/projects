...
export class PerformanceTestFramework {
  ...
  constructor(configPath?: string) {
    ...
    this.isMockMode = process.env.PERF_MOCK === 'true' || process.env.PERF_MOCK === '1';
  }

  /**
   * Run comprehensive performance test suite
   */
  async runPerformanceTests(context: TestExecutionContext): Promise<PerformanceTestResult[]> {
    console.log('🚀 Starting performance test suite...');
    
    const scenarios = this.getLoadTestScenarios();
    const results: PerformanceTestResult[] = [];
    
    // Ensure services are ready (skip in mock mode)
    if (this.isMockMode) {
      console.log('🧪 PERF_MOCK=true: Skipping service readiness checks');
    } else {
      await this.ensureServicesReady(context);
    }
    
    // Warm up services (skip in mock mode)
    if (this.isMockMode) {
      console.log('🧪 PERF_MOCK=true: Skipping warm-up');
    } else {
  ...
  private async runLoadTestScenario(
    ...
    // Start system monitoring (skip in mock mode)
    const monitoringProcess = this.isMockMode ? null : this.startSystemMonitoring(scenarioArtifactDir);
  ...
  private async executeLoadTest(
    ...
    if (this.isMockMode) {
      // Generate synthetic Artillery-like output and system metrics
      const outputFile = path.join(artifactDir, 'artillery-output.json');
      const systemMetricsFile = path.join(artifactDir, 'system-metrics.json');

      const avgLatency = Math.max(20, Math.min(200, 50 + Math.random() * 100));
      const p95Latency = avgLatency * 3;
      const p99Latency = avgLatency * 5;
      const rpsMean = Math.max(5, Math.min(300, scenario.virtualUsers * 5 + 10));
      const requestsCompleted = Math.floor(rpsMean * Math.max(1, scenario.duration - scenario.rampUpTime));
      const successes = Math.max(1, Math.floor(requestsCompleted * 0.995));
      const errors = Math.max(0, requestsCompleted - successes);
      const scenariosCompleted = Math.max(1, Math.floor(successes / Math.max(1, scenario.virtualUsers)));

      const artilleryLike = {
        aggregate: {
          latency: { min: avgLatency / 2, max: p99Latency, mean: avgLatency, p50: avgLatency * 0.8, p95: p95Latency, p99: p99Latency },
          rps: { mean: rpsMean },
          requestsCompleted,
          scenariosCreated: scenariosCompleted,
          scenariosCompleted,
          codes: { 200: successes },
          errors
        }
      ...
      try {
        fs.writeFileSync(outputFile, JSON.stringify(artilleryLike, null, 2));
      } catch {}

      // Write simple synthetic system metrics
      const metrics: any[] = [];
      const samples = Math.max(5, Math.min(60, scenario.duration));
      for (let i = 0; i < samples; i++) {
        metrics.push({
          timestamp: new Date(Date.now() + i * 1000).toISOString(),
          memory: {
            heapUsed: 120 + Math.random() * 30,
            heapTotal: 180 + Math.random() * 40,
            external: 40 + Math.random() * 20,
            rss: 160 + Math.random() * 40
          },
          cpu: {
            user: 100000 + i * 5000 + Math.floor(Math.random() * 1000),
            system: 50000 + i * 3000 + Math.floor(Math.random() * 500)
          },
          system: {
            loadAverage: [0.5 + Math.random() * 0.5, 0.7 + Math.random() * 0.5, 0.9 + Math.random() * 0.5],
            freeMemory: 2048 + Math.random() * 512,
            totalMemory: 8192
          }
        });
      }
      try {
        fs.writeFileSync(systemMetricsFile, JSON.stringify(metrics, null, 2));
      } catch {}

      return artilleryLike;
    }
   ...
   */
  private createArtilleryConfig(scenario: LoadTestScenario, context: TestExecutionContext): string {
    const baseUrl = context.environment_config.services.backend.base_url;
   ...
   */
  private startSystemMonitoring(artifactDir: string): ChildProcess {
   ...
   */
  private createMonitoringScript(artifactDir: string): string {
    return `
const fs = require('fs');
...
`;
   ...
   */
  private async collectMetrics(artifactDir: string, loadTestResult: any): Promise<PerformanceMetrics> {
   ...
   */
  private parseArtilleryResults(result: any): Partial<PerformanceMetrics> {
   ...
   */
  private async parseSystemMetrics(artifactDir: string): Promise<Partial<PerformanceMetrics>> {
    const metricsFile = path.join(artifactDir, 'system-metrics.json');
   ...
   */
  private async loadBaseline(scenarioName: string, environment: string): Promise<PerformanceBaseline | null> {
   ...
   */
  private generateHtmlReport(results: PerformanceTestResult[]): string {
...
<body>
  ...
  // Helper methods
  private getLoadTestScenarios(): LoadTestScenario[] {
    const longRunEnabled = process.env.RUN_PERF_LONG === 'true' || process.env.RUN_PERF_LONG === '1';
    ...
    if (!longRunEnabled) {
      const quick = { ...scenarios[0], duration: 15, virtualUsers: 3, rampUpTime: 3, thresholds: { ...scenarios[0].thresholds, throughput: { minRequestsPerSecond: 5 } } };
      console.log('⏩ RUN_PERF_LONG not set: running quick local perf scenario only');
      return [quick];
    }
  ...
  private async ensureServicesReady(context: TestExecutionContext): Promise<void> {
  ...
  private async warmUpServices(context: TestExecutionContext): Promise<void> {
  ...
  private calculateCpuUsage(cpuMetrics: any[]): number {
  ...
  private logScenarioResult(result: PerformanceTestResult): void {
  ...
  private async collectArtifacts(artifactDir: string): Promise<string[]> {
