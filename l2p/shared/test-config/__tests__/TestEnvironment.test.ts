...
describe('TestEnvironment', () => {
  ...
  describe('Environment Lifecycle', () => {
    it('should start the test environment successfully', async () => {
      // Mock successful Docker operations with healthy service output
    ...
    it('should stop the test environment successfully', async () => {
      // Mock start prerequisites
      const mockSpawn = require('child_process').spawn;
      mockSpawn.mockImplementation(() => ({
        stdout: {
          on: jest.fn((event, callback) => {
            if (event === 'data') callback('healthy');
          })
        },
        stderr: { on: jest.fn() },
        on: jest.fn((event, callback) => {
      ...
      // Mock successful stop (reuse same mock)
  ...
  describe('Error Handling', () => {
    it('should handle Docker command failures gracefully', async () => {
      ...
      // Prevent cleanup from executing real commands after failure
      jest.spyOn(testEnv as any, 'cleanup').mockResolvedValue(undefined);
