...
interface MockTestResult {
  ...
  summary: {
    ...
    keyboardNavigationPassed: number;
    keyboardNavigationFailed: number;
    colorContrastPassed: number;
    colorContrastFailed: number;
    overallScore: number;
  };
  artifacts: string[];
}
...
describe('AccessibilityTestFramework', () => {
  ...
  test('should create keyboard navigation tests', () => {
    const tests = (accessibilityFramework as any).getKeyboardNavigationTests();
  ...
  test('should validate ARIA attributes correctly', () => {
    ...
    for (const validation of validations) {
      const result = (accessibilityFramework as any).validateAriaAttribute(
  ...
  test('should calculate accessibility summary correctly', () => {
    const mockViolations = [
      { id: 'test1', impact: 'critical' as const, description: 'Critical issue', help: 'Fix it', helpUrl: 'http://example.com', tags: [], nodes: [] },
      { id: 'test2', impact: 'serious' as const, description: 'Serious issue', help: 'Fix it', helpUrl: 'http://example.com', tags: [], nodes: [] },
      { id: 'test3', impact: 'moderate' as const, description: 'Moderate issue', help: 'Fix it', helpUrl: 'http://example.com', tags: [], nodes: [] },
      { id: 'test4', impact: 'minor' as const, description: 'Minor issue', help: 'Fix it', helpUrl: 'http://example.com', tags: [], nodes: [] }
    ];

    const mockKeyboardResults = [
      { test: {} as any, passed: true, errors: [], screenshots: [] },
      { test: {} as any, passed: false, errors: ['Error'], screenshots: [] }
    ];

    const mockColorContrastResults = [
      { selector: '.test1', expectedRatio: 4.5, actualRatio: 5.0, passed: true, wcagLevel: 'AA' as const },
      { selector: '.test2', expectedRatio: 4.5, actualRatio: 3.0, passed: false, wcagLevel: 'AA' as const }
    ];
    ...
    const summary = (accessibilityFramework as any).calculateSummary(
      mockViolations,
      mockKeyboardResults,
      mockColorContrastResults,
      mockAriaResults,
      mockScreenReaderResults
    );
  ...
  test('should get score class correctly', () => {
    const getScoreClass = (accessibilityFramework as any).getScoreClass as (score: number) => string;
  ...
  test('should sanitize URL correctly', () => {
    const sanitizeUrl = (accessibilityFramework as any).sanitizeUrl as (url: string) => string;
  ...
  test('should generate HTML report structure', () => {
    ...
    const htmlReport = (accessibilityFramework as any).generateHtmlReport(mockResults as MockTestResult[]);
  ...
  test('should handle empty results gracefully', () => {
    ...
    const htmlReport = (accessibilityFramework as any).generateHtmlReport(emptyResults as MockTestResult[]);
