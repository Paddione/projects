...
describe('CoverageReporter', () => {
    ...
    describe('exportFormats', () => {
        it('should export coverage in multiple formats', async () => {
            const formats = ['html', 'json', 'lcov', 'xml'];
            const exports = await coverageReporter.exportFormats(mockCoverageData[0], [...formats], tempDir);
    ...
    describe('aggregateCoverageData', () => {
        ...
        it('should aggregate multiple coverage reports correctly', async () => {
            ...
            expect(mockFs.writeFileSync).toHaveBeenCalled();
            const writeCall = mockFs.writeFileSync.mock.calls.find(call => call[0].toString().includes('coverage-report.json'));
    ...
    describe('HTML report generation', () => {
        it('should generate valid HTML report', async () => {
            ...
            const htmlWriteCall = mockFs.writeFileSync.mock.calls.find(call => call[0].toString().includes('coverage-report.html'));
    ...
    describe('LCOV report generation', () => {
        it('should generate valid LCOV report', async () => {
            ...
            const lcovWriteCall = mockFs.writeFileSync.mock.calls.find(call => call[0].toString().includes('lcov.info'));
            expect(lcovWriteCall).toBeDefined();
            if (lcovWriteCall) {
                const lcovContent = lcovWriteCall[1];
    ...
    describe('Badge generation', () => {
        it('should generate coverage badge SVG', async () => {
            ...
            const badgeWriteCall = mockFs.writeFileSync.mock.calls.find(call => call[0].toString().includes('coverage-badge.svg'));
            expect(badgeWriteCall).toBeDefined();
            if (badgeWriteCall) {
                const svgContent = badgeWriteCall[1];
    ...
    describe('Historical data handling', () => {
        it('should save historical data when includeHistorical is true', async () => {
            ...
            const historicalWriteCall = mockFs.writeFileSync.mock.calls.find(call => call[0].toString().includes('historical-coverage.json'));
