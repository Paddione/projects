...
describe('CoverageConfigManager', () => {
  let coverageManager: CoverageConfigManager;
  let tempDir: string;
  ...
  beforeEach(() => {
    ...
    (CoverageConfigManager as any).instance = undefined;
  ...
  describe('collectAndAggregateCoverage', () => {
    beforeEach(() => {
      ...
      mockFs.existsSync.mockImplementation(((path: fs.PathLike) => {
        ...
        return p.includes('coverage') || p.includes('.kiro');
      }) as any);
  ...
  describe('configuration persistence', () => {
    it('should save configuration to file', () => {
      ...
      expect(mockFs.writeFileSync).toHaveBeenCalledWith(
        expect.stringContaining('coverage-config.json'),
        expect.stringContaining('"statements": 95')
      );
    });
