...
export interface InfrastructureAnalysis {
  ...
  redundantConfigurations: RedundantConfig[];
...
export interface RedundantConfig {
  type: 'jest' | 'playwright' | 'docker' | 'script';
  files: string[];
  reason: string;
  action: 'merge' | 'remove' | 'consolidate';
}
...
export interface ConfigConsolidation {
  type: string;
  sourceFiles: string[];
  targetFile: string;
  action: string;
...
export class InfrastructureAnalyzer {
   ...
   */
  async analyzeInfrastructure(): Promise<InfrastructureAnalysis> {
   ...
   */
  private async findDuplicateFiles(categories: TestCategories): Promise<DuplicateFileGroup[]> {
   ...
   */
  private async analyzeDuplicateGroup(fileName: string, files: any[]): Promise<DuplicateFileGroup | null> {
   ...
   */
  private async findRedundantConfigurations(): Promise<RedundantConfig[]> {
    const redundantConfigs: RedundantConfig[] = [];
   ...
   */
  private async isScriptUsed(scriptPath: string): Promise<boolean> {
    const configFiles = [
      'package.json',
   ...
   */
  private generateRecommendations(analysis: InfrastructureAnalysis): string[] {
   ...
   */
  async generateCleanupPlan(analysis: InfrastructureAnalysis): Promise<CleanupPlan> {
