...
export class TestConfigManager {
  ...
  public static getInstance(configPath?: string): TestConfigManager {
  ...
  public loadConfig(): TestConfig {
    try {
      ...
      this.config = yaml.load(configContent) as TestConfig;
   ...
   */
  public getEnvironmentConfig(environment: TestEnvironmentType): TestEnvironmentConfig {
    if (!this.config) {
      this.loadConfig();
    }
   ...
   */
  public getTestTypeConfig(testType: TestType): TestTypeConfig {
    if (!this.config) {
      this.loadConfig();
    }
   ...
   */
  public getGlobalConfig(): GlobalTestConfig {
    if (!this.config) {
      this.loadConfig();
    }
   ...
   */
  public createExecutionContext(environment: TestEnvironmentType, testType: TestType): TestExecutionContext {
    if (!this.config) {
      this.loadConfig();
    }
   ...
   */
  public validateConfig(config?: TestConfig): ValidationResult {
   ...
   */
  public setupEnvironmentVariables(environment: TestEnvironmentType): void {
   ...
   */
  public async performHealthCheck(environment: TestEnvironmentType): Promise<TestEnvironmentStatus> {
    const envConfig = this.getEnvironmentConfig(environment);
   ...
   */
  public getJestConfig(context: TestExecutionContext, isBackend: boolean = true): any {
  ...
  private validateTestTypeConfig(
    typeName: string,
  ...
  private async checkServiceHealth(serviceName: string, serviceConfig: ServiceConfig): Promise<HealthCheckResult> {
    ...
    try {
      ...
      const response = await Promise.race([
        ...
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Timeout')), serviceConfig.timeout * 1000)
        )
      ]) as Response;
  ...
  private async checkDatabaseConnection(databaseUrl: string): Promise<boolean> {
    try {
  ...
  private getCoveragePatterns(isBackend: boolean): string[] {
    if (isBackend) {
      return [
        'src/**/*.ts',
        '!src/**/*.d.ts',
