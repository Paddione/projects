...
program
    ...
    try {
        ...
        if (options.statements)
            thresholds.statements = parseInt(options.statements, 10);
        if (options.branches)
            thresholds.branches = parseInt(options.branches, 10);
        if (options.functions)
            thresholds.functions = parseInt(options.functions, 10);
        if (options.lines)
            thresholds.lines = parseInt(options.lines, 10);
...
program
    ...
    try {
        ...
        const warnings = [];
        // Validate thresholds
        const validateThresholds = (thresholds, name) => {
