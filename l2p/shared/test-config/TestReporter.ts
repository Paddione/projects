...
import { TestResult, CoverageReport, CoverageMetrics } from './TestRunner.js';
...
export type ReportFormat = 'html' | 'json' | 'xml' | 'markdown' | 'console';
...
export class TestReporter {
   ...
   */
  aggregateResults(results: TestResult[], startTime: Date = new Date()): TestSummary {
   ...
   */
  aggregateCoverage(coverageReports: CoverageReport[]): CoverageMetrics | undefined {
   ...
   */
  collectArtifacts(results: TestResult[]): ArtifactCollection {
  ...
  async generateReport(
    ...
    const reportData: TestReportData = {
      summary,
      results,
      coverage: results.map(r => r.coverage).filter(Boolean) as CoverageReport[],
      artifacts,
      environment,
      timestamp: new Date(),
   ...
   */
  private generateHtmlReport(data: TestReportData, outputPath: string): string {
...
<body>
   ...
   */
  private generateCoverageMetric(label: string, metric: { covered: number; total: number; percentage: number }): string {
   ...
   */
  private generateJsonReport(data: TestReportData, outputPath: string): string {
    const jsonData = {
      ...data,
      summary: {
        ...data.summary,
   ...
   */
  private generateXmlReport(data: TestReportData, outputPath: string): string {
...
`).join('')}
   ...
   */
  private generateMarkdownReport(data: TestReportData, outputPath: string): string {
...
`;
   ...
   */
  private generateConsoleReport(data: TestReportData): string {
    const output = `
╔══════════════════════════════════════════════════════════════════════════════╗
║                                TEST REPORT                                   ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
`;
  ...
  private openInBrowser(filePath: string): void {
    ...
    open(filePath).catch((error: Error) => {
