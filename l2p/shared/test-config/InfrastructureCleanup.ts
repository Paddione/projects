...
import { InfrastructureAnalyzer, CleanupPlan, InfrastructureAnalysis } from './InfrastructureAnalyzer';

export interface CleanupResult {
  success: boolean;
  filesRemoved: string[];
  directoriesRemoved: string[];
  configurationsConsolidated: string[];
  migrationsMade: string[];
  errors: string[];
  warnings: string[];
  spaceSaved: number;
}
...
export class InfrastructureCleanup {
   ...
   */
  async executeCleanup(options: Partial<CleanupOptions> = {}): Promise<CleanupResult> {
   ...
   */
  private async removeDuplicateFiles(
    plan: CleanupPlan, 
   ...
   */
  private async consolidateConfigurations(
    plan: CleanupPlan, 
    result: CleanupResult, 
    opts: CleanupOptions
  ): Promise<void> {
   ...
   */
  private async createBackup(plan: CleanupPlan, backupDir: string): Promise<void> {
    ...
    await fs.promises.writeFile(
      path.join(backupPath, 'cleanup-manifest.json'),
   ...
   */
  private async performConfigConsolidation(consolidation: any): Promise<void> {
   ...
   */
  private displayCleanupPlan(plan: CleanupPlan, dryRun: boolean): void {
   ...
   */
  private displayCleanupResults(result: CleanupResult, dryRun: boolean): void {
