 ...
 *
 * Usage:
 *   npx tsx scripts/import_sprite_pack.ts [sprite_pack_png_dir]
 *
...
function main() {
  ...
  for (const char of CHARACTERS) {
    const destDir = path.join(RENDERS_DIR, "characters", char.arenaId);
    ensureDir(destDir);
  ...
  for (const [category, mappings] of tileCategories) {
    ...
    for (const m of mappings) {
      const destDir = path.join(RENDERS_DIR, category, m.arenaId);
      ensureDir(destDir);
