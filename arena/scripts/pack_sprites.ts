 ...
 *
 * Usage:
 *   npx tsx scripts/pack_sprites.ts [--category <name>]
 */
...
async function packCategory(category: string, manifest: Manifest) {
  const renderDir = path.join(RENDERS_DIR, category);
  ...
  try {
    ...
    for (const result of results) {
      if (result.name.endsWith('.png')) {
        const pngPath = path.join(OUTPUT_DIR, `${category}.png`);
        ...
        const jsonPath = path.join(OUTPUT_DIR, `${category}.json`);
...
async function main() {
  ...
  for (const category of categories) {
    ...
    await packCategory(category, manifest);
  }

  console.log('\n[DONE] Sprite sheets saved to', OUTPUT_DIR);
}
