...
import { undoService, type UndoEntry } from '@/services/undo-service';
 ...
 * Integrates with the undo service to show an undo button
 */
interface UndoToastOptions {
  ...
  undoId: string;
  undoType: UndoEntry['type'];
  undoDescription: string;
  undoCallback: () => Promise<void>;
  timeout?: number;
}

function toastWithUndo(options: UndoToastOptions) {
  const {
    title,
    description,
    variant = 'default',
  ...
  const cleanup = undoService.register(undoId, undoType, undoDescription, undoCallback, {
    timeout,
  });
      ...
      onClick: () => {
        undoService
          ...
          .then(() => {
            ...
            toast({
              title: 'Undone',
              description: `${undoDescription} has been undone`,
            });
          })
          .catch((error) => {
  ...
  // Clean up undo operation when toast is dismissed
