...
export interface ResourceInfo {
  id: string;
  name: string;
  type: 'container' | 'volume' | 'network' | 'image';
  size?: string;
  created: Date;
  inUse: boolean;
}
...
export class ResourceCleanup {
  ...
  constructor(projectName: string = 'learn2play-test', composeFile: string = 'docker-compose.test.yml') {
   ...
   * Stop and remove containers
   */
  private async stopContainers(result: CleanupResult, options: CleanupOptions): Promise<void> {
    try {
   ...
   * Remove volumes
   */
  private async removeVolumes(result: CleanupResult, options: CleanupOptions): Promise<void> {
    try {
   ...
   * Remove networks
   */
  private async removeNetworks(result: CleanupResult, options: CleanupOptions): Promise<void> {
    try {
   ...
   * Remove images
   */
  private async removeImages(result: CleanupResult, options: CleanupOptions): Promise<void> {
    try {
   ...
   */
  private async removeOrphanedResources(result: CleanupResult, options: CleanupOptions): Promise<void> {
    try {
   ...
   */
  private async preserveLogs(result: CleanupResult): Promise<void> {
    try {
   ...
   */
  private async listContainers(): Promise<ResourceInfo[]> {
    try {
      const output = await this.executeCommand('docker', [
   ...
   */
  private async listVolumes(): Promise<ResourceInfo[]> {
    try {
      const output = await this.executeCommand('docker', [
   ...
   */
  private async listNetworks(): Promise<ResourceInfo[]> {
    try {
      const output = await this.executeCommand('docker', [
   ...
   */
  private async listImages(): Promise<ResourceInfo[]> {
    try {
      const output = await this.executeCommand('docker', [
   ...
   * Calculate total space freed
   */
  private async calculateSpaceFreed(): Promise<string> {
    try {
   ...
   */
  async gentleCleanup(): Promise<CleanupResult> {
