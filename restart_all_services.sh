...
# VLLM containers
if [ -f "$SCRIPT_DIR/vllm/rag/docker-compose.yml" ]; then
    print_info "Stopping VLLM RAG stack..."
    cd "$SCRIPT_DIR/vllm/rag"
    docker-compose stop
...
# Auth containers
if [ -f "$SCRIPT_DIR/auth/docker-compose.yml" ]; then
...
if [ -f "$SCRIPT_DIR/vllm/manage_dashboard.sh" ]; then
    bash "$SCRIPT_DIR/vllm/manage_dashboard.sh"
